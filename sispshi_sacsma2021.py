...
class spotpy_2(object):
    ...
    def __init__(self, area, dt, PME, ETP, Qjus, idx, idx_cal, Qmon=None, fobj='DRMS'):
        self.area = area
        self.dt = dt
        self.PME = PME
        self.ETP = ETP
    ...
    def simulation(self, x):
        DF_params_1 = pd.read_csv('sispshi_params1.csv', index_col=0)
        params = {}
        params['UZTWM'] = x.UZTWM
        params['UZFWM'] = x.UZFWM
        params['LZTWM'] = DF_params_1.loc['parLZTWM'].valor
        params['LZFPM'] = DF_params_1.loc['parLZFPM'].valor
        params['LZFSM'] = DF_params_1.loc['parLZFSM'].valor
        params['ADIMP'] = x.ADIMP
        params['UZK']   = x.UZK
        params['LZPK']  = DF_params_1.loc['parLZPK'].valor
        params['LZSK']  = DF_params_1.loc['parLZSK'].valor
        params['ZPERC'] = x.ZPERC
        params['REXP']  = x.REXP
        params['PCTIM'] = x.PCTIM
        params['PFREE'] = DF_params_1.loc['parPFREE'].valor
        params['K']     = x.K
        params['N']     = x.N
        Qsim, Qbfp, Qbfs, Qtci, Qtco = simulacao(self.area, self.dt, self.PME, self.ETP, params, self.Qmon)
        Qsim = pd.Series(index=self.idx, data=Qsim)
    ...
    def objectivefunction(self, simulation, evaluation):
        ...
        # DRMS
        if self.fobj == 'DRMS':
            df_cal['fmin'] = df_cal.apply(lambda x: (x['qsim'] - x['qobs'])**2, axis=1)
            fmin = math.sqrt(df_cal['fmin'].sum())/len(df_cal)
...
class spotpy_3(object):
    ...
    def simulation(self, x):
        DF_params_2 = pd.read_csv('sispshi_params2.csv', index_col=0)
        params = {}
        params['UZTWM'] = DF_params_2.loc['parUZTWM'].valor
        params['UZFWM'] = DF_params_2.loc['parUZFWM'].valor
        params['LZTWM'] = x.LZTWM
        params['LZFPM'] = x.LZFPM
        params['LZFSM'] = x.LZFSM
        params['ADIMP'] = DF_params_2.loc['parADIMP'].valor
        params['UZK']   = DF_params_2.loc['parUZK'].valor
        params['LZPK']  = x.LZPK
        params['LZSK']  = x.LZSK
        params['ZPERC'] = DF_params_2.loc['parZPERC'].valor
        params['REXP']  = DF_params_2.loc['parREXP'].valor
        params['PCTIM'] = x.PCTIM
        params['PFREE'] = x.PFREE
        params['K']     = x.K
        params['N']     = x.N
