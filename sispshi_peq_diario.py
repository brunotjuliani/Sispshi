...
for bacia_nome, bacia_informacoes in bacias_cab.items():
    ...
    try:
        siprec = pd.read_csv(f'../Dados/PME_Siprec/{cod_bacia}_siprec_hist.csv',
                             index_col = 0, parse_dates=True)
    ...
    dados_diario = (dados_peq.resample("D", closed='left').
                  agg({'pme':np.sum, 'siprec':np.sum, 'etp':np.sum,
                       'qjus':np.mean, 'qmon':np.mean}))
    dados_diario = dados_diario.iloc[1:]

    dados_diario['pme'] = dados_diario['pme'].apply('{:.2f}'.format)
    dados_diario['siprec'] = dados_diario['siprec'].apply('{:.2f}'.format)
    dados_diario['qjus'] = dados_diario['qjus'].apply('{:.3f}'.format)
    dados_diario['etp'] = dados_diario['etp'].apply('{:.3f}'.format)
    dados_diario['qmon'] = dados_diario['qmon'].apply('{:.3f}'.format)

    with open(f'../Dados/PEQ/{cod_bacia}_{bacia_nome}_peq_diario.csv', 'w', newline = '') as file:
        writer = csv.writer(file)
        writer.writerow([area_inc])
    dados_diario.to_csv(f'../Dados/PEQ/{cod_bacia}_{bacia_nome}_peq_diario.csv', mode = 'a',
                     date_format='%Y-%m-%dT%H:%M:%S+00:00', sep = ",")
    print('Finalizado PEQ diario - ' + cod_bacia + ' - ' + bacia_nome)
