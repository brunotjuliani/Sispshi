...
import math
import smap
from smap import Xnomes, Xmin, Xmax
import dds
...
def fobj_nse(X):
    params = X
    Qsim = smap.SMAP(area, dt, PME, ETP, params)
...
dt = 86400 # segundos / 24 horas / 1 dia
...
fig.add_trace(go.Scatter(x=Simul.index, y=Simul['S_NSE'], name='Qsim - NSE', marker_color='green'), row=2, col=1)
fig.update_yaxes(title_text='Chuva [mm]', row=1, col=1)
fig.update_yaxes(title_text='Vazão [m3s-1]', row=2, col=1)
fig.update_layout(legend_title_text='Comparação Modelo SMAP')
...
df_params.to_csv(f'./Parametros/param_smap_{bn}_{bnome}.csv', index=False)

Simul = Simul.loc['2018':'2019']

print('Nash S_NSE = ' + str(he.nse(Simul['S_NSE'],Simul['Qobs'])))
print('Log-Nash S_NSE = ' + str(he.nse(np.log(Simul['S_NSE']),np.log(Simul['Qobs']))))
print('PBIAS S_NSE = ' + str(hv.evaluator(hv.pbias,Simul['S_NSE'],Simul['Qobs'])))
